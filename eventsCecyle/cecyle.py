...
dico[2][2]=["comptage", "#f30279"]
dico[2][3]=["densité cellulaire", "#f30279"]
dico[2][4]=["taille des boîtes", "#f30279"]
dico[2][5]=["ensemencement", "#f30279"]
dico[3]={}
dico[3][-1]=["transition : expérience", "#0028D5"]
dico[4]={}
dico[4][-1]=["transition : résultats", "#aa00d5"]
dico[4][0]=["comptages", "#aa00d5"]
dico[4][1]=["correction courbe croissance", "#aa00d5"]
dico[4][2]=["proportion des phases", "#aa00d5"]
dico[4][3]=["correction des durées", "#aa00d5"]
dico[5]={}
...
def time_decode(timeStampZero, timeStamp):
	...
	return date-dateZ
with open('champo_'+str(sys.argv[1])+'.log') as data_file:    
    data = json.load(data_file)

print (len(data['events']), "events loaded")

f = open('data'+str(sys.argv[1])+'.csv', 'w')
